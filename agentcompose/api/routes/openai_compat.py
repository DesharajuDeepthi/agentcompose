...
from agentcompose.api.thread_id import resolve_thread_id, OPENWEBUI_CONFIG
...
from agentcompose.api.streaming import stream_openai_response_with_interrupt
...
async def create_chat_completion(
    ...
    1. **Normal flow**: User sends message → AgentCompose processes → Response returned
    2. **Interrupt flow** (when send_message is used):
       - AgentCompose needs user input → Returns message as normal response
    ...
    This makes AgentCompose work as a drop-in replacement for OpenAI's API while
...
async def list_models() -> Dict[str, Any]:
    ...
    Returns a single "agentcompose" model that represents the orchestration system.
    """
    return {
        "object": "list",
        "data": [
...
async def get_model(model_id: str) -> Dict[str, Any]:
    ...
    Always returns "agentcompose" model info regardless of model_id.
    """
    return {
        "id": "agentcompose",
        "object": "model",
